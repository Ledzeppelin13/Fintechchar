...
บรรทัดที่ 73-74  เป็นการตรวจสอบว่า ```Adoption.sol``` ได้รับการ deploy แล้วหรือไม่ ถ้าเรียบร้อยแล้ว ให้ทำฟังก์ชั่น Instance **(App.contracts.Adoption.deployed().then(function (instance))** โดยนำค่าที่ deploy แล้ว ไปใส่ในตัวแปร instance ซึ่ง ณ ตอนนี้ก็คือ```Adoption.sol``` ไปใส่ใน ```adoptionInstance``` 

บรรทัดที่ 76-77 เมื่อ function ```getAdopter``` ทำงานแล้ว ให้นำพารามิเตอร์ที่ได้ไปให้กับ function ```adopters``` ซึ่งจะทำงานต่อในบรรทัดที่ 78-80   
บรรทัดที่ 78-80  เป็นfunction ```For loop``` ซึ่งนำเอาข้อมูลเด็กทั้ง 16 จากบล็อกเชนมาเชื่อมต่อกับ back end คือ app.js  แปลความได้ว่า **เมื่อมีผู้อุปการะแล้วให้เปลี่ยนปุ่ม "รับอุปการะ" เป็น "อุปการะแล้ว"**

[![appjs4.png](https://i.postimg.cc/XNDxm0Nd/appjs4.png)](https://postimg.cc/7b0gJd2Z)
